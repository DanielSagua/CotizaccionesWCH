...
async function changeEstadoWithHistoryAndComment({ id_solicitud, id_estado, actorUserId, comentario, meta }) {
  const pool = await getPool();
  const trx = new sql.Transaction(pool);
  ...
  try {
    // 1) Estado actual + nombre nuevo (para resumen)
    const beforeR = await trx.request()
      .input('id', sql.Int, id_solicitud)
      .query(`
        SELECT TOP 1 s.id_estado, e.nombre AS estado_nombre
        FROM dbo.Solicitudes s
        INNER JOIN dbo.EstadosSolicitud e ON e.id_estado = s.id_estado
        WHERE s.id_solicitud = @id
      `);

    const before = beforeR.recordset[0];
    if (!before) throw new Error('Solicitud no encontrada');

    const targetR = await trx.request()
      .input('id_estado', sql.Int, id_estado)
      .query(`SELECT TOP 1 nombre FROM dbo.EstadosSolicitud WHERE id_estado=@id_estado`);

    const target = targetR.recordset[0];
    ...
    // 3) Historial con justificación
    const cambios = {
      before: { id_estado: before.id_estado, estado: before.estado_nombre },
      after: { id_estado, estado: target.nombre },
    ...
    await trx.request()
      .input('id_solicitud', sql.Int, id_solicitud)
      .input('accion', sql.VarChar, 'CHANGE_STATUS')
      .input('resumen', sql.NVarChar, `Estado: ${before.estado_nombre} → ${target.nombre}`)
      .input('cambios_json', sql.NVarChar, JSON.stringify(cambios))
      .input('actor_user_id', sql.Int, actorUserId)
      .input('ip', sql.VarChar, meta?.ip || null)
      .input('user_agent', sql.NVarChar, meta?.userAgent || null)
      .query(`
        INSERT INTO dbo.SolicitudesHistorial
          (id_solicitud, accion, resumen, cambios_json, actor_user_id, ip, user_agent)
        VALUES
          (@id_solicitud, @accion, @resumen, @cambios_json, @actor_user_id, @ip, @user_agent)
      `);

    // 4) Comentario (queda en el chat)
    await trx.request()
      .input('id_solicitud', sql.Int, id_solicitud)
      .input('actor_user_id', sql.Int, actorUserId)
